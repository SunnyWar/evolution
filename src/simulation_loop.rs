...
pub fn run_simulation(generations: usize, pop_size: usize, params: &SimParams) -> Vec<f64> {
    let mut population: Vec<Agent> = (0..pop_size)
        .map(|_| Agent {
            latent_fitness: rand::random::<f64>() * 2.0,
            traits: VisibleTraits {
                intelligence: 0.3 + rand::random::<f64>() * 1.2, // start low, but wider
                physical_size: 0.5 + rand::random::<f64>() * 2.0,
                appearance_delta: rand::random::<f64>(),
            },
            fitness_score: 0.0,
        })
        .collect();

    // Initialize RNG once for the whole simulation
    ...
    for _generation in 0..generations {
        ...
        while new_population.len() < pop_size {
            ...
            child.traits.intelligence += (rand::random::<f64>() - 0.5) * 0.05;
            child.traits.intelligence = child.traits.intelligence.max(0.0);
            child.traits.physical_size += (rand::random::<f64>() - 0.5) * 0.1;
            child.traits.appearance_delta += (rand::random::<f64>() - 0.5) * 0.05;
            child.latent_fitness += (rand::random::<f64>() - 0.5) * 0.05;
